...
async function setData(key, val) {
  ...
  try {
    ...
    await window._setDoc(window._doc(window._db, 'data', key), { value: JSON.stringify(val) });
  } catch(e) { console.error('Firebase write error', e); }
}

async function loadAllFromFirebase() {
  if (!window._db) return;
  const keys = ['users', 'leaderboard', 'testRequests', 'dataVersion'];
  for (const key of keys) {
    try {
      const snap = await window._getDoc(window._doc(window._db, 'data', key));
      if (snap.exists()) _cache[key] = JSON.parse(snap.data().value);
    } catch(e) { console.error('Firebase read error', key, e); }
  }
}

function subscribeToData(key, callback) {
  if (!window._db) return;
  window._onSnapshot(window._doc(window._db, 'data', key), snap => {
    if (snap.exists()) {
      _cache[key] = JSON.parse(snap.data().value);
      callback(_cache[key]);
    }
  });
}
...
async function initFromMaster() {
  const savedVersion = getData('dataVersion', 0);
  const masterVersion = MASTER_DATA.dataVersion || 0;

  if (masterVersion > savedVersion) {
    // Firebase is empty or outdated — seed from MASTER_DATA
    if (MASTER_DATA.leaderboard) await setData('leaderboard', MASTER_DATA.leaderboard);
    if (MASTER_DATA.users)       await setData('users',       MASTER_DATA.users);
    await setData('dataVersion', masterVersion);
  }
  // Ensure all keys exist
  if (!getData('users', null))        await setData('users', {});
  if (!getData('leaderboard', null))  await setData('leaderboard', []);
}
...
window.onload = async function() {
  ...
  loadDiv.id = 'fbLoading';
  loadDiv.style.cssText = 'position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(10,15,30,0.95);display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:9999;';
  loadDiv.innerHTML = '<div style="font-family:Rajdhani,Arial,sans-serif;font-size:1.5rem;font-weight:700;letter-spacing:3px;color:#60a5fa;margin-bottom:1rem;">⚔️ LOADING...</div><div style="font-size:0.85rem;color:#64748b;letter-spacing:2px;">Connecting to server</div>';
  ...
  // Seed from MASTER_DATA if Firebase is empty or outdated
  await initFromMaster();
  migrateOldTierCodes();

  // Subscribe to live real-time updates
  subscribeToData('leaderboard', () => {
    if (document.getElementById('lbList')) renderLeaderboard();
    if (document.getElementById('adminLbList')) renderAdminLeaderboard();
    if (document.getElementById('playersTableBody')) renderPlayersTable();
  });
  subscribeToData('users', () => {
    if (currentUser) {
      const users = getData('users', {});
      const key = currentUser.username.toLowerCase();
      if (users[key]) currentUser = users[key];
      if (document.getElementById('profileContent')) renderProfile();
      if (document.getElementById('applyContent')) renderApply();
    }
    if (isAdmin && document.getElementById('requestsList')) renderAdminRequests();
  });
